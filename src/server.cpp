...
void writedemo(int chan, void *data, int len)
{
    ...
    MUTEX_LOCK( &writedemoMutex );
    int stamp[3] = { gamemillis, chan, len };
    lilswap(stamp, 3);
    demorecord->write(stamp, sizeof(stamp));
    demorecord->write(data, len);
    MUTEX_UNLOCK( &writedemoMutex );
}
...
int canspawn(client *c)   // beware: canspawn() doesn't check m_arena!
{
    ...
    if(!c || c->type == ST_EMPTY || !c->isauthed || !team_isvalid(c->team) ||
        ...
        (servmillis - c->connectmillis < 1000 + c->state.reconnections * 2000 &&
          gamemillis > 10000 && totalclients > 3 && !team_isspect(c->team))) { spawn_type = SP_OK_NUM; goto finish; } // equivalent to SP_DENY
    if(!c->isonrightmap) { spawn_type = SP_WRONGMAP; goto finish; };
    if(mastermode == MM_MATCH && matchteamsize)
    {
        if(c->team == TEAM_SPECT || (team_isspect(c->team) && !m_teammode)) { spawn_type = SP_SPECT; goto finish; }
        if(c->team == TEAM_CLA_SPECT || c->team == TEAM_RVSF_SPECT)
        {
            if(numteamclients()[team_base(c->team)] >= matchteamsize) { spawn_type = SP_SPECT; goto finish; }
            else { spawn_type = SP_REFILLMATCH; goto finish; }
        }
    }
    finish:
    double *var = ( double* ) Lua::getFakeVariable( LUA_YIELD_CAN_SPAWN, NULL, LUA_TNUMBER, "ii", c->clientnum, spawn_type );
    if ( var != NULL )
    {
      spawn_type = (int) var[0];
      delete[] var;
    }
    return spawn_type;
}
    ...
    else
    {
        ...
        if(ms)
        {
            ...
            loopi(smapstats.hdr.numents)
            {
                ...
                server_entity se = { e.type, false, false, false, 0, smapstats.entposs[i * 3], smapstats.entposs[i * 3 + 1], smapstats.entposs[i * 3 + 2] }; /// Lua mod
                sents.add(se);
...
int getbantype(int cn)
{
    ...
    int banned = BAN_NONE;
    if(checkgban(c.peer->address.host)) banned = BAN_MASTER;
    if(banned == BAN_NONE && ipblacklist.check(c.peer->address.host)) banned = BAN_BLACKLIST;
    if ( banned == BAN_NONE ) loopv(bans)
    {
        ban &b = bans[i];
        if(b.millis < servmillis) { bans.remove(i--); }
        if(b.address.host == c.peer->address.host) { banned = b.type; break; }
    }
    double *var = ( double* ) Lua::getFakeVariable( LUA_YIELD_IS_BANNED, NULL, LUA_TNUMBER, "si", iptoa( c.peer->address.host ), banned );
    if ( var != NULL )
    {
      banned = (int) var[0];
      delete[] var;
...
struct voteinfo
{
    ...
    void end(int result, bool fromLua = false)
    {
        ...
        if ( !fromLua && Lua::callHandler( LUA_ON_VOTE_END, "iiisii", result, owner, type, text, num1, num2 ) == Lua::PLUGIN_BLOCK ) result = VOTE_NO;
        ...
        if ( !fromLua ) Lua::callHandler( LUA_ON_VOTE_END_AFTER, "iiisii", result, owner, type, text, num1, num2 );
    }
...
bool scallvote(voteinfo *v, ENetPacket *msg) // true if a regular vote was called
{
    ...
    double *var = ( double* ) Lua::getFakeVariable( LUA_ON_PLAYER_CALL_VOTE, NULL, LUA_TNUMBER, "iisiii", v->owner, v->type, v->text, v->num1, v->num2, error );
    if ( var != NULL )
    {
      error = (int) var[0];
      delete[] var;
...
void disconnect_client(int n, int reason, bool fromLua)
{
    ...
    if ( !fromLua )
        if ( Lua::callHandler( LUA_ON_PLAYER_DISCONNECT, "ii", n, reason ) == Lua::PLUGIN_BLOCK ) return;
    sdropflag(n);
    client &c = *clients[n];
...
void welcomepacket(packetbuf &p, int n)
{
    ...
    if(!c || clients.length()>1)
    {
        ...
        {
            client &c = *clients[i];
            if(c.type==ST_EMPTY || c.clientnum==n) continue;
            putint(p, c.clientnum);
...
void process(ENetPacket *packet, int sender, int chan)
{
    ...
    while((curmsg = p.length()) < p.maxlen)
    {
        ...
        switch(type)
        {
            ...
            case SV_AKIMBO:
            {
                Lua::callHandler( LUA_ON_PLAYER_GET_AKIMBO, "i", cl->clientnum );
...
//                 if (cl) disconnect_client(cl->clientnum, DISC_TAGT); // remove this in the future, when auth is complete
            ...
            case SV_POS:
            {
                ...
                vec vel, o; bool onfloor = cl->state.onfloor; ///
                loopi(3) o.v[i] = cl->state.o[i]; ///
                if (cl->g & 1) vel.x = getint(p)/DVELF; else vel.x = 0;
                if ((cl->g>>1) & 1) vel.y = getint(p)/DVELF; else vel.y = 0;
                if ((cl->g>>2) & 1) vel.z = getint(p)/DVELF; else vel.z = 0;
                if ((cl->g>>3) & 1) getint(p);
                cl->f = getuint(p);
                cl->state.onfloor = (cl->f >> 4) & 1; ///
                cl->state.onladder = (cl->f >> 5) & 1; ///
                cl->state.crouching = (cl->f >> 7) & 1; ///
                cl->state.scoping = (cl->g >> 4) & 1; ///
                if(!cl->isonrightmap) break;

                loopi(3)
                {
                    float dr = cl->state.o.v[i] - o.v[i];
                    if ( !dr ) cl->state.vel.v[i] = 0.0f;
                    else if ( cl->state.vel.v[i] ) cl->state.vel.v[i] = dr * 0.05f + cl->state.vel.v[i] * 0.95f;
                    cl->state.vel.v[i] += vel.v[i];
                    if ( i==2 && onfloor && cl->state.vel.v[i] < 0.0f ) cl->state.vel.v[i] = 0.0f;
                }
            ...
            case SV_POSC:
            {
                ...
                int zt = q.getbits(s);
                if(negz) zt = -zt;
                int g1 = cl->state.scoping = q.getbits(1); /// scoping
            ...
            case SV_EXTENSION:
            {
                ...
                if ( Lua::callHandler( LUA_ON_EXTENSION, "sis", ext, sender, text ) != Lua::PLUGIN_BLOCK )
                {
                    if ( !strcmp( ext, "lua::reload" ) )
                    {
            ...
            {
                int val = getint(p);
                cl->state.state = val ? CS_EDITING : CS_ALIVE;
                Lua::callHandler( LUA_ON_PLAYER_SWITCH_EDITING, "ib", cl->clientnum, ( val > 0 ) );
                QUEUE_MSG;
                break;
            }
        ...
        Lua::callHandler( LUA_ON_SERVER_PROCESS_AFTER, "iii", cl->clientnum, chan, type );
    }
...
void checkintermission()
{
    ...
    {
      if ( !forceintermission ) Lua::callHandler( LUA_ON_MAP_END, "" );
...
#endif
    int diff = framemillis = nextmillis - servmillis;
    gamemillis += diff;
...
//        if(m_lms) lmscheck();
    ...
    if(forceintermission || ((smode>1 || (gamemode==0 && nonlocalclients)) && gamemillis-diff>0 && (gamemillis + intermission_check_shear)/60000!=(gamemillis + intermission_check_shear -diff)/60000))
    ...
    Lua_lastMeasured[0] = serverhost->totalSentData;
    Lua_lastMeasured[1] = serverhost->totalReceivedData;
    ...
    sendworldstate();
    Lua::callHandler( LUA_ON_LOOP_AFTER, "" );
}
...
int getpongflags(enet_uint32 ip)
{
    ...
    int banned = BAN_NONE;
    loopv(bans) if(bans[i].address.host == ip) { banned = bans[i].type; break; }
    double *var = ( double* ) Lua::getFakeVariable( LUA_YIELD_IS_BANNED, NULL, LUA_TBOOLEAN, "si", iptoa( ip ), banned );
    if ( var != NULL )
    {
      banned = (int) var[0];
      delete[] var;
    }
    if ( banned ) flags |= 1 << PONGFLAG_BANNED;
...
void initserver(bool dedicated, int argc, char **argv)
{
    ...
    Lua::initialize( lua_scripts_path );
    if ( hitreg_fixed ) logline( ACLOG_INFO, "<Lua> Hitreg fix enabled" );
    else logline( ACLOG_INFO, "<Lua> Hitreg fix disabled" );
...
int main(int argc, char **argv)
{
    ...
    MUTEX_RECURSIVE_INIT( &writedemoMutex );
    MUTEX_RECURSIVE_INIT( &sendpacketMutex );
